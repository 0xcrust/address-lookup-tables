//! epoch-report-hack modifies the dumped EpochReportAccount so that its `epoch` field reads as `0`.
...
const EPOCH_REPORT_PATH: &str =
    "../fixtures/accounts/sunrise-stake/epoch-report-account.json";
...
fn modify_sunrise_epoch_report() -> anyhow::Result<()> {
    ...
    log::info!(
        "Modified epoch report account info at path {} successfully!",
        EPOCH_REPORT_PATH
    );
